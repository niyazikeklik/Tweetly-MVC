...
namespace Tweetly_MVC.Init
{
    public static class Login
    {
        ...
        {
            driver.LinkeGit("https://mobile.twitter.com/login", WaitForLoad: false);
            if (driver.Url.Contains("flow"))
            {

                Thread.Sleep(2500);
                ((IWebElement)driver.JsRun("return document.querySelector('[name=username]');")).SendKeys(ka + Keys.Enter);
                Thread.Sleep(2500);
                ((IWebElement)driver.JsRun("return document.querySelector('[name=password]');")).SendKeys(ps + Keys.Enter);
                Thread.Sleep(2500);
